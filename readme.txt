=== Namaz Vakitleri ===
Contributors: Erdem Arslan @erdemsaid
Tags: diyanet, namaz, ezan, vakit, muslim, islam, salah, salat, mosque, masjid, prayer, worship
Requires at least: 2.8 or higher
Tested up to: 3.8
Stable tag: 2.0.3
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Donate link: http://www.erdemarslan.com/

This plugin shows prayer (salah) times for Muslims with pretty widget.
Bu eklenti namaz vakitlerini widget üzerinde gösterir.

== Description ==

Bu eklenti, Diyanet İşleri Başkanlığının yayınladığı namaz vakitlerini Wordpress sitenizde güzel bir bileşen olarak
yayınlamanızı sağlar. Varsayılan bir şehir seçebilirsiniz. Esnek bir yapıya sahip bu eklenti
sayesinde ziyaretçilerinize namaz vakitlerini göstermekte hiç zorlanmayacaksınız.

Karşılaştığınız hatalarda lütfen beni bilgilendiriniz.

**Özellikler**

* İstediğiniz kadar şehir, ilçe için namaz vakitlerini gösterebilme
* %100 Diyanetin verileri
* 8 farklı renk seçeneğini kullanabilirsiniz
    
**Kullanım**

1. Önce Eklentiyi kurun.
2. Eklentiyi aktifleştirin.
3. Ayarlar -> Namaz Vakti Ayarlarına gidin ve seçimlerinizi yapın.
4. Görünüm -> Bileşenler sayfasına gidin ve bileşeni side bar'a sürükleyip kaydedin.
5. İsterseniz Ayarlar -> Namaz Vakti Ayarlarından şehirlerinizi özelleştirebilirsiniz.

**Hak Sahipleri**

Diyanet İşleri Başkanlığı - <a href="http://www.diyanet.gov.tr">Ziyaret Et</a>

== Installation ==

İsterseniz yönetim panelinden eklentiyi kurabilir ya da namazvakti klasörünü FTP üzerinden dosyaları yükleyerek, yönetim panelinizden aktifleştirebilirsiniz.

== Frequently Asked Questions == 


== Screenshots ==

1. Bileşen (Widget) Görüntüsü 1
2. Bileşen (Widget) Görüntüsü 2
3. Yönetim Paneli 1
4. Yönetim Paneli 2
5. Yönetim Paneli 3


== Contact ==

Benimle iletişime geçmek için lütfen <a href="http://www.erdemarslan.com">blogumu</a> kullanınız.

== Changelog ==

= 2.0.2 =
Eklenti güncelleme hatası giderildi.

= 2.0.1 =
Dil hatası düzeltildi.

= 2.0 =
Eklenti tekrardan yazılarak, lisans anahtarı kaldırıldı.
İlçeler de sisteme dahil edildi.
Görünüm tamamen yenilendi.

= 1.4 =
Widget üzerindeki tarih gösterimi ile ilgili sıkıntı giderildi.
Önbellekleme ile ilgili problem giderildi.

= 1.3 =

http://api.eralabs.net/ bağlantı problemi giderildi.
Verilerin işlenmesi ile ilgili kontroller geliştirildi.
Bazı ufak hatalar giderildi.

= 1.2 =

Dosya yerleşim hataları giderildi.

= 1.1 =

Widget için renk ayarı eklendi.

= 1.0.2 =

Eklenti hazırlandı.
Görünüm iyileştirmeleri yapıldı.

== UPGRADE NOTICE ==

* Güncelleme yok

== License ==

Namaz Vakti is released under the General Public License v2 or later.
For more information please read<a href="http://www.gnu.org/copyleft/gpl.html">http://www.gnu.org/copyleft/gpl.html